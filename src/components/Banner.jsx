...
function Banner() {
  ...
  return (
    <div className="relative m-auto h-fit flex w-full justify-start items-center overflow-visible">
      ...
      <img className={"bg-contain transition-opacity ease-in-out w-full"} src={data.banner.images[number - 1]} alt="" />
      <div
        className={
      ...
      >
        ...
        <h1 className="w-full lg:text-4xl md:text-xl sm:text-lg">{data.banner?.title}</h1>
        <p className={"lg:text-xl md:text-md text-xs w-full text-start"}>{data.banner?.statement}</p>
      </div>
